...
# --- VÁ LỖI SKIMAGE VÀ LIBSVM CHO IMQUALITY ---
import skimage.color
_orig_rgb2gray = skimage.color.rgb2gray
def _safe_rgb2gray(rgb, *args, **kwargs):
    if hasattr(rgb, 'ndim') and rgb.ndim == 2:
        return rgb
    return _orig_rgb2gray(rgb, *args, **kwargs)
skimage.color.rgb2gray = _safe_rgb2gray
try:
    import libsvm.svmutil as svmutil
    import libsvm.svm as svm
    svmutil.PRECOMPUTED = getattr(svm, 'PRECOMPUTED', 4)
except Exception:
    pass
# -----------------------------------------------
